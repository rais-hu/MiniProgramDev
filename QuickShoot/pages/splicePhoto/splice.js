...
Page({
  data: {
    ...
    readuSave:false,
    frameList:[],
    downloadFrame:'https://636c-cloud1-7g3jdaee863f7829-1305999227.tcb.qcloud.la/frame/frame1.png'
  },
  onLoad: function (option) {
    ...
    wx.cloud.database().collection('frames').get()
    .then(res=>{
      console.log("获取云数据库中的相框成功",res)
      this.setData({
        frameList:res.data
      })
    })
    .catch(res=>{
      console.log("获取云数据库中的相框失败",res)
    })
  },
  ...
  chooseFrame(e){
    ...
    wx.getImageInfo({
      ...
      success: function (res) {
        ...
        console.log("相框：",self.data.frameSrc)
      }
    })
  },
  closeFrameChoose(){
  ...
  saveImgToPhone(){
    ...
    if (self.data.page === 'longImages'){ //拼长图
    ...
    } else if (self.data.page === 'photoFrame'){ // 相框重画图
      var frameHeight = self.frameHeight ? self.frameHeight : 1000 * self.deviceRatio  
      self.setData({
      ...
      // 尝试用getImagInfo试试
      // wx.getImageInfo({
      //   src: '/image/frame/frame1.png',
      //   success: function (re) {
      //     // console.log("获取相框getImageInfo成功",re)
      //   self.setData({
      //     downloadFrame:re.path
      //   })
      //   },
      //   fail:function (re) {
      //     // console.log("获取相框getImageInfo失败",re)
      //   }
      // })
      // wx.cloud.downloadFile({
      //   fileID: self.data.frameSrc
      // }).then(res => {
      //   // get temp file path
      //   console.log("从服务器上下载相框",res.tempFilePath)
      //   self.setData({
      //     downloadFrame:res.tempFilePath
      //   })
      // }).catch(error => {
      //   // handle error
      // })

      //  ===============================问题就出在这个drawImage不能画一个网络图片
      //先画照片
      tempCtx.drawImage(self.data.photoSrc, sX, sY, sW, sH, canvasX, canvasY, canvasW, canvasH)
      //再画相框
      // console.log("再画相框",self.data.frameSrc)

      tempCtx.drawImage(self.data.frameSrc, 0, 0, canvasW, canvasH) //self.data.downloadFrame
      // console.log("在画相框的下面")
      tempCtx.draw()
      setTimeout(function(){
        wx.canvasToTempFilePath({
